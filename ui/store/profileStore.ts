    ...
    try {
      ...
      if (res.ok) {
        ...
        const teamData = {
          label: team.teamname,
          scores: submissions
            .filter((sub: SubmissionType) => sub.correct === true)
        ...
        const teamGraph = processScores(
          [teamData],
          useEventStore.getState().info.event_start.toUTCString()
        );
        const userSubmissions = submissions.filter(
          (sub: SubmissionType) => sub.userid === useAuthStore.getState().user.userid
        );

        var userCorrect = userSubmissions.filter(
          (sub: SubmissionType) => sub.correct === true
        ).length;
        var userScore = userSubmissions
          .filter(
            (sub: SubmissionType) =>
              sub.correct === true && sub.userid === useAuthStore.getState().user.userid
          )
          .reduce((acc: number, sub: SubmissionType) => acc + sub.points, 0);
        ...
        set({
          team: { ...team, submissions: submissions },
          teamGraph: teamGraph,
          userSubmissionsProgress: {
            correct: userCorrect,
            incorrect: userSubmissions.length - userCorrect,
          },
          teamSubmissionsProgress: {
            correct: teamCorrect,
            incorrect: submissions.length - teamCorrect,
          },
          userCategoryProgress: userCategoryProgress,
          teamCategoryProgress: teamCategoryProgress,
        });
      } else {
        const response = await res.json();
